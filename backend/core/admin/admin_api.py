...
@router.post("/chromaflow_agent-agents/install-user/{account_id}")
async def admin_install_chromaflow_agent_for_user(
    account_id: str,
    replace_existing: bool = False,
    _: bool = Depends(verify_admin_api_key)
):
    """Install ChromaFlow Agent agent for a specific user."""
    logger.debug(f"Admin installing ChromaFlow Agent agent for user: {account_id}")
    
    service = ChromaFlow AgentDefaultAgentService()
    agent_id = await service.install_chromaflow_agent_agent_for_user(account_id, replace_existing)
    
    if agent_id:
        return {
            "success": True,
            "message": f"Successfully installed ChromaFlow Agent agent for user {account_id}",
            "agent_id": agent_id
        }
    else:
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to install ChromaFlow Agent agent for user {account_id}"
        )
