...
class AgentConfigTool(AgentBuilderBaseTool):
    ...
    async def update_agent(
        ...
        try:
            ...
            # Enforce ChromaFlow Agent restrictions (simplified)
            if is_chromaflow_agent_default:
                ...
                if restricted_fields:
                    return self.fail_response(
                        f"Cannot modify {', '.join(restricted_fields)} for ChromaFlow Agent. "
                        f"ChromaFlow Agent's core identity is centrally managed. You can still add MCPs and triggers."
                    )
