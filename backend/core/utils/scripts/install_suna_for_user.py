#!/usr/bin/env python3
"""
ChromaFlow Agent Agent Installation Script for Individual Users

Simple script to install ChromaFlow Agent agents for users by email address.

Usage:
    # Install ChromaFlow Agent for a user
    python install_chromaflow_agent_for_user.py user@example.com
    
    # Install with replacement (if agent already exists)
    python install_chromaflow_agent_for_user.py user@example.com --replace

Examples:
    python install_chromaflow_agent_for_user.py john.doe@company.com
    python install_chromaflow_agent_for_user.py admin@example.org --replace
"""
...
class ChromaFlow AgentUserInstaller:
    def __init__(self):
        self.db = DBConnection()
        self.service = ChromaFlow AgentDefaultAgentService(self.db)
    ...
    async def install_for_email(self, email: str, replace: bool = False):
        ...
        print(f"🚀 Installing ChromaFlow Agent agent...")
        agent_id = await self.service.install_chromaflow_agent_agent_for_user(
            account_id, 
            replace_existing=replace
        )
        
        if agent_id:
            print(f"✅ Successfully installed ChromaFlow Agent agent!")
            print(f"   🤖 Agent ID: {agent_id}")
            print(f"   👤 User: {email}")
            print(f"   📦 Account: {account_id}")
        else:
            print(f"❌ Failed to install ChromaFlow Agent agent for {email}")



async def main():
    parser = argparse.ArgumentParser(
        description="Install ChromaFlow Agent agent for a user by email",
    ...
    parser.add_argument('--replace', action='store_true', 
                       help='Replace existing ChromaFlow Agent agent if present')
    
    args = parser.parse_args()
    
    installer = ChromaFlow AgentUserInstaller()
