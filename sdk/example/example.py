...
from chromaflow.utils import print_stream
...
async def main():
    ...
    chromaflow_client = chromaflow.ChromaFlow(
        os.getenv("CHROMAFLOW_API_KEY", "pk_xxx:sk_xxx"),
    ...
    if not agent_id:
        agent = await chromaflow_client.Agent.create(
            name="Generic Agent",
        ...
        agent = await chromaflow_client.Agent.get(agent_id)
        await agent.update(allowed_tools=["get_weather"])

    # Setup the thread
    thread_id = kv.get("thread_id")
    if not thread_id:
        thread = await chromaflow_client.Thread.create()
        kv.set("thread_id", thread._thread_id)
    else:
        thread = await chromaflow_client.Thread.get(thread_id)
