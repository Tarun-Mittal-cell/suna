...
import { useChromaFlowTeamTemplates, useInstallTemplate } from '@/hooks/react-query/secure-mcp/use-secure-mcp';
...
export function CustomAgentsSection({ onAgentSelect }: CustomAgentsSectionProps) {
  ...
  const { data: templates, isLoading, error } = useChromaFlowTeamTemplates();
  ...
  const handleCardClick = (template: any) => {
    const marketplaceTemplate: MarketplaceTemplate = {
      ...
      is_chromaflow_team: template.is_chromaflow_team || false,
