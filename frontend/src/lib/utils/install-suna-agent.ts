...
async function installChromaFlowAgentForNewUser(userId: string) {
  try {
    ...
    if (response.ok) {
      ...
      console.error(`Failed to install ChromaFlow Agent for user:`, errorData);
      return false;
    }
  } catch (error) {
    console.error('Error installing ChromaFlow Agent for new user:', error);
    return false;
  }
}
...
export async function checkAndInstallChromaFlow AgentAgent(userId: string, userCreatedAt: string) {
  ...
  if (userCreatedDate > tenMinutesAgo) {
    const installKey = `chromaflow-agent-install-attempted-${userId}`;
    if (typeof window !== 'undefined' && localStorage.getItem(installKey)) {
      return;
    }
    
    const success = await installChromaFlowAgentForNewUser(userId);
